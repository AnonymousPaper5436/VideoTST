...
from ..gadgets.my_metrics import Accuracy, Scalar, IoU
    ...
    for split in ["train", "val"]:
        for task, flag in pl_module.hparams.config["loss_names"].items():
            if flag:
                ...
                if task == "kfqa":
                    setattr(pl_module, f"{split}_start_loss", Scalar())
                    setattr(pl_module, f"{split}_start_accuracy", Accuracy())
                    setattr(pl_module, f"{split}_end_loss", Scalar())
                    setattr(pl_module, f"{split}_end_accuracy", Accuracy())
                    setattr(pl_module, f"{split}_iou", IoU())
...
def epoch_wrapup(pl_module):
    ...
    for task, flag in pl_module.hparams.config["loss_names"].items():
        ...
        if task == 'kfqa':
            m = getattr(pl_module, f"{phase}_start_loss")
            pl_module.log(f"{task}/{phase}/start_loss_epoch", m.compute(), rank_zero_only=True)
            m.reset()
            m = getattr(pl_module, f"{phase}_start_accuracy")
            pl_module.log(f"{task}/{phase}/start_accuracy_epoch", m.compute(), rank_zero_only=True)
            m.reset()
            
            m = getattr(pl_module, f"{phase}_end_loss")
            pl_module.log(f"{task}/{phase}/end_loss_epoch", m.compute(), rank_zero_only=True)
            m.reset()
            m = getattr(pl_module, f"{phase}_end_accuracy")
            pl_module.log(f"{task}/{phase}/end_accuracy_epoch", m.compute(), rank_zero_only=True)
            m.reset()
            
            m = getattr(pl_module, f"{phase}_iou")
            pl_module.log(f"{task}/{phase}/iou", m.compute(), rank_zero_only=True)
            m.reset()
